...
public static class ObjectToolbarPlacement
{
        ...
        double viewportWidth, double viewportHeight)
    {
        ...
        return new ToolbarPlacement(
            ...
            ClampToViewport(top, toolbarHeight, viewportHeight),
            below);
    }
    ...
    private static double ClampToViewport(double position, double size, double extent)
    {
        double max = extent - EdgeMargin - size;
        if (max <= EdgeMargin) { return EdgeMargin; }
        return Math.Clamp(position, EdgeMargin, max);
    }
}
