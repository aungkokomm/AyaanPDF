...
impl<'a> PdfPageTextObject<'a> {
    ...
    /// The raw `FPDF_PAGEOBJECT` handle for this text object.
    ///
    /// AYAAN PDF FORK ADDITION (not in upstream 0.8.37): exposed so glyph indices
    /// produced by a shaping engine (rustybuzz/HarfBuzz) can be applied with
    /// `FPDFText_SetCharcodes`, which is how complex scripts (Burmese, Devanagari,
    /// Arabic) are drawn as correct, selectable vector text.
    #[inline]
